...
export default function CreatePostPage() {
  ...
  return (
    <div className="min-h-screen bg-white max-w-md mx-auto relative font-sans flex flex-col">
      {/* Premium Glass Header */}
      <div className="sticky top-0 z-30 bg-white/80 backdrop-blur-xl px-4 py-4 flex items-center justify-between border-b border-zinc-100/50">
        <div className="flex items-center gap-3">
          <Link href="/" className="w-10 h-10 flex items-center justify-center rounded-full bg-zinc-50 hover:bg-zinc-100 transition-colors active:scale-95 text-zinc-800">
            <ArrowLeft size={20} />
          </Link>
          <span className="font-extrabold text-[16px] tracking-tight text-zinc-900">New Post</span>
        </div>
      </div>
      ...
      <div className="flex-1 px-5 py-6 flex flex-col relative">
        ...
        {error && (
          <div className="bg-red-50 text-red-500 px-4 py-3 rounded-2xl text-[13px] font-medium mb-4 flex items-center gap-2">
        ...
        <div className="flex gap-4 flex-1">
          ...
          <div className="shrink-0 pt-1">
            <div className="relative">
              <div className="w-11 h-11 rounded-full overflow-hidden bg-zinc-100 border border-zinc-200">
                {userProfile?.avatar_url ? (
                  <Image src={userProfile.avatar_url} alt="You" width={44} height={44} className="w-full h-full object-cover" />
                ) : (
                  <div className="w-full h-full flex items-center justify-center bg-gradient-to-tr from-zinc-100 to-zinc-200 text-zinc-500 font-bold text-lg">
            ...
            {/* Connection Line */}
            <div className="w-px h-full min-h-[100px] bg-zinc-100 mx-auto mt-2"></div>
          </div>
          
          <div className="flex-1 flex flex-col min-w-0">
             <div className="mb-1 flex items-center gap-1.5">
               <span className="font-bold text-[15px] tracking-tight">{userProfile?.full_name || userProfile?.username || "Loading..."}</span>
               <span className="text-[12px] text-[#222]/50 font-medium">· Just now</span>
             </div>
              <textarea
                ref={textareaRef}
                autoFocus
                ...
                className="w-full text-[17px] text-zinc-900 border-none outline-none resize-none min-h-[30vh] bg-transparent pt-1 placeholder:text-zinc-400 font-medium leading-[1.6]"
              />

             {imagePreview && (
               <div className="relative mt-4 rounded-2xl overflow-hidden border border-zinc-100 group">
              ...
              {/* Integrated Toolbar */}
              <div className="mt-8 flex items-center justify-between py-2 border-t border-zinc-100/50 pt-4">
                ...
                <button
                  ...
                  className={`px-8 py-2.5 rounded-full font-bold text-[14px] tracking-wide transition-all duration-300 flex items-center gap-2 shadow-sm ${
                    (content.trim() || imageFile)
                    ? "bg-[#E5FF66] text-zinc-900 shadow-[0_4px_15px_rgba(229,255,102,0.4)] hover:scale-105 active:scale-95" 

    ...
    <div className="min-h-screen bg-[#F8F9FA] pb-[100px] max-w-md mx-auto relative font-sans">
      ...
      {modal === "terms" && (
        <DocModal title="Terms of Service" onClose={() => setModal(null)}>
          ...
          <p><strong className="text-zinc-900">Eligibility:</strong> You must be a currently enrolled student at a verified university to use Campus.</p>
          <p><strong className="text-zinc-900">Account Responsibility:</strong> You are responsible for maintaining the confidentiality of your account and password and for restricting access to your device.</p>
          <p><strong className="text-zinc-900">Content:</strong> You retain ownership of content you post. By posting, you grant Campus a non-exclusive license to display your content to other users.</p>
          <p><strong className="text-zinc-900">Prohibited Activities:</strong> You may not use Campus for illegal activities, harassment, spam, or any activity that violates our Community Guidelines.</p>
          <p><strong className="text-zinc-900">Termination:</strong> We reserve the right to terminate or suspend access to our service without prior notice for conduct that we believe violates these Terms.</p>
          <p className="text-zinc-400 text-[12px]">Last updated: March 2026</p>
        </DocModal>
      )}
      ...
      {modal === "privacy" && (
        <DocModal title="Privacy Policy" onClose={() => setModal(null)}>
          ...
          <p><strong className="text-zinc-900">Data We Collect:</strong> We collect information you provide (name, email, university), as well as usage data (posts, interactions, device info).</p>
          <p><strong className="text-zinc-900">How We Use It:</strong> Your data is used to provide and improve our services, personalize your feed, and keep the platform safe.</p>
          <p><strong className="text-zinc-900">Data Sharing:</strong> We do not sell your personal data. We may share data with trusted partners to operate our services, subject to confidentiality agreements.</p>
          <p><strong className="text-zinc-900">Your Rights:</strong> You may request deletion of your account and associated data at any time by contacting support.</p>
          <p><strong className="text-zinc-900">Cookies:</strong> We use cookies and similar tracking technologies to improve your experience.</p>
          <p className="text-zinc-400 text-[12px]">Last updated: March 2026</p>
        </DocModal>
      )}
      ...
      {modal === "oss" && (
        <DocModal title="Open Source Libraries" onClose={() => setModal(null)}>
          {[
            ...
            <div key={i} className="bg-zinc-50 rounded-2xl px-4 py-3">
              <div className="flex items-center justify-between">
                <span className="font-black text-zinc-900">{lib.name}</span>
                <span className="text-[12px] font-bold text-zinc-400 bg-zinc-200 px-2 py-0.5 rounded-md">{lib.license}</span>
              </div>
              <p className="text-[12px] text-zinc-500 mt-1">v{lib.version} · {lib.author}</p>
            </div>
          ))}
        </DocModal>
      )}

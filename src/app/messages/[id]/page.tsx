    ...
    <div className="flex flex-col h-dvh bg-[#F8F9FA] dark:bg-black max-w-md mx-auto relative font-sans overflow-hidden overscroll-none transition-colors">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-8 flex flex-col gap-6 scrollbar-hide">
        <div className="flex justify-center mb-4">
          <span className="text-[10px] font-bold uppercase tracking-widest text-zinc-400 dark:text-zinc-600 bg-zinc-100 dark:bg-zinc-900 px-3 py-1 rounded-full">Conversation Started</span>
        </div>
        ...
        {messages.map((msg) => {
          ...
          return (
            ...
            >
              {!isMe && (
                <div className="w-8 h-8 rounded-full overflow-hidden shrink-0 mb-1 bg-zinc-100 dark:bg-zinc-900 flex items-center justify-center">
                  {partner?.avatar_url ? (
                    <Image 
                      src={partner.avatar_url} 
                      alt="Avatar" 
              ...
              <div 
                className={`max-w-[82%] rounded-[22px] transition-all overflow-hidden ${
                  ...
                  isMe 
                    ? isImage ? "" : "bg-zinc-900 dark:bg-white text-white dark:text-black rounded-br-none" 
                    : isImage ? "" : "bg-white dark:bg-zinc-900 text-zinc-800 dark:text-zinc-100 rounded-bl-none border border-zinc-100/50 dark:border-zinc-800"
                }`}
              >
                {msg.content.startsWith('[VOICE_NOTE]') ? (
                  <div className="flex flex-col gap-1">
                   ...
                   <div 
                     ...
                     className="relative bg-zinc-100 dark:bg-zinc-900 flex items-center justify-center"
                   >
                      <img 
                      ...
                      />
                      {msg.content.split('\n\n')[1] && (
                        <div className={`p-4 text-[14px] leading-relaxed ${isMe ? 'bg-zinc-900 dark:bg-white text-white dark:text-black' : 'bg-white dark:bg-zinc-900 text-zinc-800 dark:text-zinc-100 border-t border-zinc-100 dark:border-zinc-800'}`}>
                           {msg.content.split('\n\n')[1]}
                        </div>
                      )}
                  ...
                  <>
                    <p className="text-[14.5px] leading-relaxed font-medium">{msg.content}</p>
                    <span className={`block text-[9px] mt-1.5 font-black uppercase tracking-widest ${isMe ? "text-white/40 dark:text-black/40 text-right" : "text-zinc-400 dark:text-zinc-600"}`}>
                      {new Date(msg.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                    </span>
                  </>
                )}
        ...
        >
          ...
          <div className="flex items-center gap-2 text-zinc-400">
            <button 
              ...
              className={`w-10 h-10 flex items-center justify-center rounded-full hover:bg-zinc-50 dark:hover:bg-zinc-900 transition-colors ${(isUploading || pendingImage) ? 'opacity-30 cursor-not-allowed' : ''}`}
            >
              <Plus size={22} />
            </button>
          </div>
